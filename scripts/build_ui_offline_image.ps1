param(
    [string]$ImageTag = "a-cong-ocr-ui:chandra",
    [string]$ArchivePath = ".\dist\a-cong-ocr-ui_chandra.tar",
    [string]$DockerfilePath = "Dockerfile.ui",
    [switch]$SkipManifest,
...
try {
    ...
    if (-not $SkipManifest) {
        ...
        $manifest = [ordered]@{
            ...
            image_role = "web-and-playground"
            validation_commands = @(
                ".\.venv\Scripts\python.exe -m pytest tests",
                "python -m uvicorn app.main:app --host 127.0.0.1 --port 5000",
                "python -m uvicorn app.playground_proxy:app --host 127.0.0.1 --port 5001"
            )
        }
        $manifest | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $manifestPath -Encoding UTF8
        Write-Host "Wrote UI image manifest: $manifestPath"
    }
}
finally {
    Pop-Location
