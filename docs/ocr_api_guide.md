...
### 2.6 비동기 요청 runtime 메타데이터

`POST /api/v1/ocr`, `POST /api/v1/marker`로 생성된 완료/실패 결과에는 운영 추적용 `runtime` 객체가 포함된다.

| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `request_id` | string | 요청 ID |
| `request_kind` | string | `ocr` 또는 `marker` |
| `file_name` | string | 처리 파일명 |
| `file_size_bytes` | integer | 입력 파일 크기 |
| `started_at` | string | ISO 시각 |
| `finished_at` | string | ISO 시각 |
| `duration_ms` | number | 처리 시간 |
| `page_count` | integer \| null | 처리 페이지 수 |
| `status` | string | `complete` 또는 `failed` |
| `error_code` | string | 실패 시 분류 코드 |
...
| `GET` | `/api/v1/capabilities` | 동기 | 지원 입력/출력/기능 조회 |
...
| `DELETE` | `/api/v1/requests` | 동기 | 오래된 비동기 요청 보관함 정리 |
...
| `include_raw` | form-data | boolean | N | `true` | `false`면 `raw_vl`, `raw_structure`, `raw_fallback_ocr` 제외 |
...
| `markdown` | string | 제목/문단 중심 markdown |
| `block_count` | integer | 블록 수 |
| `blocks` | array | OCR 블록 목록 |
| `raw_vl` | object | VL OCR 원본 payload. `include_raw=false`면 제외 |
...
| `max_pages` | form-data | integer | N | - | 앞에서부터 처리할 최대 페이지 수 |
| `page_range` | form-data | string | N | - | zero-based page index/range. 예: `0,2-4` |
| `include_raw` | form-data | boolean | N | `true` | `false`면 raw OCR payload 제외 |
...
| `dpi` | 1 이상 |
| `max_pages` | 1 이상 |
| `page_range` | zero-based index, 오름차순 range |
| PDF header | 첫 1024 bytes 안에 `%PDF-` 필요 |
| 빈 파일 업로드 | 허용 안 함 |
...
| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `page_count` | integer | 처리된 페이지 수 |
| `source_page_count` | integer | 원본 PDF 전체 페이지 수 |
| `processed_page_count` | integer | 처리된 페이지 수 |
| `pdf_name` | string | 업로드 파일명 |
| `text` | string | 전체 처리 페이지 텍스트 |
| `markdown` | string | 전체 처리 페이지 markdown |
...
| `file_url` | form-data | string | 조건부 | - | 로컬 경로, `file://`, `http(s)://` 입력. 파일 업로드가 없을 때 사용 |
...
입력은 `file`, `file.0`, `file_url` 중 하나가 필요하다.
...
| `max_pages` | form-data | integer | N | 없음 | 최대 처리 페이지 수 |
| `page_range` | form-data | string | N | 없음 | 선택 페이지 범위 |
| `mode` | form-data | string | N | `balanced` | `fast`, `balanced`, `accurate` |
| `output_format` | form-data | string | N | `markdown` | 출력 포맷. `json,markdown`처럼 쉼표로 복수 지정 가능 |
| `paginate` | form-data | boolean | N | `false` | 페이지 사이에 페이지 구분자를 넣음 |
| `add_block_ids` | form-data | boolean | N | `false` | HTML 결과에 `data-block-id` 속성 추가 |
| `include_markdown_in_chunks` | form-data | boolean | N | `false` | `chunks`와 JSON block에 block별 markdown 추가 |
| `skip_cache` | form-data | boolean | N | `false` | 호환 옵션. 현재 로컬 Chandra 처리에는 캐시가 없으므로 메타데이터에만 기록 |
| `extras` | form-data | string | N | 없음 | Datalab 호환용 comma list. 현재는 요청 메타데이터에 기록 |
| `additional_config` | form-data | string | N | 없음 | Datalab 호환용 JSON 문자열. 현재는 요청 메타데이터에 기록 |
...
복수 포맷을 요청하면 `result`는 요청 포맷별 payload 객체가 된다. 단일 포맷이면 기존처럼 해당 포맷 payload가 바로 들어간다. 상위 `json`, `markdown`, `html`, `chunks` 필드는 호환성과 디버깅 편의를 위해 항상 함께 반환된다.
...
| `output_formats` | array[string] | 정규화된 출력 포맷 목록 |
| `markdown` | string \| null | markdown 결과 |
| `html` | string \| null | html 결과 |
| `json` | object \| null | 구조화 JSON 결과 |
| `chunks` | array \| null | chunk 결과 |
| `parse_quality_score` | number | 처리된 block confidence 평균 |
| `metadata` | object | mode, output format, Chandra backend, page_range 등 요청/처리 메타데이터 |
...
## 8. Request Cleanup API

### 8.1 `DELETE /api/v1/requests`

오래된 비동기 OCR/Marker request 디렉터리를 정리한다. 기본은 `dry_run=true`라서 삭제하지 않고 후보만 반환한다.

Query params:

| 필드 | 타입 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `older_than_hours` | number | `24` | 이 시간보다 오래된 요청만 대상 |
| `status_filter` | string \| null | 없음 | 예: `complete`, `failed` |
| `dry_run` | boolean | `true` | `false`일 때 실제 삭제 |
...
### 9.1 `GET /api/v1/thumbnails/{lookup_key}`
...
### 10.7 요청 보관함 dry-run 정리

```bash
curl -X DELETE "http://127.0.0.1:18009/api/v1/requests?older_than_hours=24&status_filter=complete&dry_run=true"
```
