...
RUNTIME_SETTING_SPECS: tuple[RuntimeSettingSpec, ...] = (
    ...
    RuntimeSettingSpec(
        ...
        value_type="bool",
        default=False,
        description="OCR 결과 품질이 낮을 때 전처리 이미지를 만들어 한 번만 더 시도합니다. 기본값은 빠른 처리를 위해 꺼져 있습니다.",
    ),
    RuntimeSettingSpec(
    ...
    RuntimeSettingSpec(
        ...
        value_type="string",
        default="ocr_layout",
        description="OCR 엔진에 전달하는 prompt type입니다. 기본값은 layout OCR에 맞춘 ocr_layout입니다.",
    ),
    RuntimeSettingSpec(
        ...
        min_value=1,
        max_value=16,
        description="로컬 runner에서 한 번에 처리할 page image 수입니다. vLLM remote 호출은 보통 1을 권장합니다.",
    ),
    RuntimeSettingSpec(
        ...
        min_value=0,
        max_value=200,
        description="Playground에서 페이지 범위를 비웠을 때 적용할 최대 쪽수입니다. 0이면 업로드한 파일 전체를 처리합니다.",
    ),
    RuntimeSettingSpec(
