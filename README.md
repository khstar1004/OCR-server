...
`Chandra OCR` 단일 백엔드로 신문 PDF/이미지를 페이지 단위 입력으로 정규화하고, 기사 단위로 묶어 결과를 저장/조회하는 서버입니다.
...
- 인터넷망/현장 검증용 compose 실행은 `remote-ocr` 프로필을 기본 경로로 봅니다.
- `.env`에 `OCR_SERVICE_URL=http://<OCR_IP>:8000` 을 넣으면 app가 해당 OCR 호스트/IP를 호출합니다.
- `remote-ocr` 프로필을 쓰면 `app + playground + ocr-service + vllm-ocr` 4컨테이너 구조로 뜹니다.
- `vllm` 프로필은 vLLM 단독 점검용으로만 둡니다. 일반 테스트는 `remote-ocr`를 사용합니다.
...
- 단일 파일 실행: `POST /api/v1/jobs/run-single?file_name=...` (`.pdf`, `.png`, `.jpg`, `.jpeg`, `.webp`)
...
- Datalab 호환 요청 정리: `DELETE /api/v1/requests`
...
- 국회 전송 사전검증: `GET /api/v1/jobs/{job_id}/news-payload`
- 국회 전송 실행: `POST /api/v1/jobs/{job_id}/deliver`
...
- OCR API: `http://127.0.0.1:18009`
- OCR Playground: `http://127.0.0.1:18109/playground/`

## 입력 파일 넣는 폴더

기본 입력 폴더는 [news_pdfs](C:\Users\USER\Desktop\a-cong-OCR-V2\news_pdfs) 입니다. PDF가 암호화되어 렌더링이 막히는 경우 페이지 이미지를 PNG/JPG/WEBP로 넣어도 같은 파이프라인으로 처리됩니다.

같이 만들어 둔 기본 폴더:

- 입력 파일: [news_pdfs](C:\Users\USER\Desktop\a-cong-OCR-V2\news_pdfs)
...
- `docker compose --profile remote-ocr up -d` 는 app, playground, ocr-service, vLLM을 같이 올립니다.
...
- 기본 URL: `http://127.0.0.1:18007` (app), `http://127.0.0.1:18109/playground/` (playground), `http://127.0.0.1:18009` (OCR service)
...
- 내부 호환 API는 `mode`, `page_range`, `max_pages`, 복수 `output_format`, `paginate`, `add_block_ids`, `include_markdown_in_chunks`, `skip_cache`, `file_url`을 받습니다.
- 완료/실패 결과에는 `runtime` 메타데이터가 포함되고, 오래된 request 보관함은 `DELETE /api/v1/requests`로 dry-run 후 정리할 수 있습니다.
